...
pub fn trialBalance(database: db.Database, book_id: i64, as_of_date: []const u8) !*ReportResult {
    ...
    const result = try buildReportResult(database, tb_sql, .{ book_id, book_id, as_of_date });
...
const is_sql: [*:0]const u8 =
    ...
    \\WHERE e.book_id = ? AND a.book_id = ?
    \\  AND e.status IN ('posted', 'reversed')
    \\  AND e.entry_type NOT IN ('opening', 'closing')
...
pub fn incomeStatement(database: db.Database, book_id: i64, start_date: []const u8, end_date: []const u8) !*ReportResult {
    try verifyBookExists(database, book_id);
    const result = try buildReportResult(database, is_sql, .{ book_id, book_id, start_date, end_date });
    errdefer result.deinit();
    result.decimal_places = try getDecimalPlaces(database, book_id);
    return result;
}
...
const ni_sql: [*:0]const u8 =
    \\SELECT a.account_type, SUM(el.base_debit_amount), SUM(el.base_credit_amount)
    \\FROM ledger_entries e
    \\JOIN ledger_entry_lines el ON el.entry_id = e.id
    \\JOIN ledger_accounts a ON a.id = el.account_id
    \\WHERE e.book_id = ? AND a.book_id = ?
...
pub fn balanceSheetAuto(database: db.Database, book_id: i64, as_of_date: []const u8) !*ReportResult {
    try verifyBookExists(database, book_id);
    const result = try buildReportResult(database, bs_sql, .{ book_id, book_id, as_of_date });
    errdefer result.deinit();
    result.decimal_places = try getDecimalPlaces(database, book_id);
    return result;
}

/// Plain balance sheet that returns only real A/L/E rows through the requested
/// as-of date. Mid-period dates are supported; callers that need a self-
/// balancing presentation should use balanceSheetWithProjectedRE.
...
pub fn balanceSheetWithProjectedRE(database: db.Database, book_id: i64, as_of_date: []const u8, fy_start_date: []const u8) !*ReportResult {
    ...
    var should_project = true;
    {
        var period_stmt = try database.prepare(
            \\SELECT status FROM ledger_periods
            \\WHERE book_id = ? AND start_date <= ? AND end_date >= ?
            \\ORDER BY start_date DESC
            \\LIMIT 1;
        );
        defer period_stmt.finalize();
        try period_stmt.bindInt(1, book_id);
        try period_stmt.bindText(2, as_of_date);
    ...
    if (should_project) {
        ...
        try stmt.bindText(3, fy_start_date);
